...
<div class="min-h-screen bg-surface text-on-surface font-body-md overflow-x-hidden">
    ...
    <main class="ml-0 md:ml-64 p-margin-mobile md:p-margin-desktop transition-all duration-300">
        <div class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                <div class="flex items-center gap-2 text-outline mb-2">
                    ...
                    <span class="font-label-bold text-label-bold uppercase tracking-widest text-primary">Edit Product</span>
                </div>
        ...
        <form id="productForm" action="{{ route('admin.comics.update', $comic) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-gutter">
            @csrf
            @method('PUT')
                ...
                <section class="bg-pure-white p-8 shadow-[0_32px_32px_rgba(32,42,54,0.04)] rounded-xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="flex flex-col gap-2">
                            ...
                            <input class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="title" required type="text" value="{{ old('title', $comic->title) }}"/>
                        </div>
                        <div class="flex flex-col gap-2">
                        ...
                        <div class="col-span-full flex flex-col gap-2">
                            ...
                            <textarea class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="description" rows="4">{{ old('description', $comic->description) }}</textarea>
                        </div>
                    </div>
                </section>
                ...
                <section class="bg-pure-white p-8 shadow-[0_32px_32px_rgba(32,42,54,0.04)] rounded-xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                <input class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="price" required type="number" step="0.01" value="{{ old('price', $comic->price) }}"/>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <input class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="stock" required type="number" value="{{ old('stock', $comic->stock) }}"/>
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="font-label-bold text-label-bold text-outline">SKU ID</label>
                            <input class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="isbn" type="text" value="{{ old('isbn', $comic->isbn) }}"/>
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="font-label-bold text-label-bold text-outline">PAGES</label>
                            <input class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="pages" type="number" value="{{ old('pages', $comic->pages) }}"/>
                        </div>
                        <div class="flex flex-col gap-2">
                            ...
                            <select class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all appearance-none" name="language" required>
                                <option value="es" {{ old('language', $comic->language) == 'es' ? 'selected' : '' }}>Español</option>
                                <option value="en" {{ old('language', $comic->language) == 'en' ? 'selected' : '' }}>English</option>
                                <option value="ja" {{ old('language', $comic->language) == 'ja' ? 'selected' : '' }}>Japanese</option>
                            </select>
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="font-label-bold text-label-bold text-outline">PUBLICATION DATE</label>
                            <input class="w-full bg-surface border-0 border-b-2 border-surface-variant py-3 font-body-md text-body-md focus:border-energy-lime transition-all" name="publication_date" type="date" value="{{ old('publication_date', $comic->publication_date?->format('Y-m-d')) }}"/>
                        </div>
                    </div>
                </section>
                ...
                <section class="bg-pure-white p-8 shadow-[0_32px_32px_rgba(32,42,54,0.04)] rounded-xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <label class="flex items-center gap-3 bg-surface p-4 rounded-lg border border-outline-variant">
                            <input class="w-5 h-5 accent-primary" name="is_featured" type="checkbox" value="1" {{ old('is_featured', $comic->is_featured) ? 'checked' : '' }}/>
                            <span class="font-bold">Featured</span>
                        </label>
                        <label class="flex items-center gap-3 bg-surface p-4 rounded-lg border border-outline-variant">
                            <input class="w-5 h-5 accent-primary" name="is_new_release" type="checkbox" value="1" {{ old('is_new_release', $comic->is_new_release) ? 'checked' : '' }}/>
                            <span class="font-bold">New Release</span>
                        </label>
                        <label class="flex items-center gap-3 bg-surface p-4 rounded-lg border border-outline-variant">
                            <input class="w-5 h-5 accent-primary" name="is_active" type="checkbox" value="1" {{ old('is_active', $comic->is_active) ? 'checked' : '' }}/>
            ...
            <div class="col-span-12 lg:col-span-4 space-y-8">
                <section class="bg-pure-white p-8 shadow-[0_32px_32px_rgba(32,42,54,0.04)] rounded-xl">
                    ...
                    @if($comic->image_path)
                        <img src="{{ asset('storage/' . $comic->image_path) }}" alt="{{ $comic->title }}" class="w-full aspect-[2/3] object-cover rounded-lg border border-outline-variant mb-4"/>
                    @endif
                    <input class="w-full bg-surface border border-outline-variant rounded-lg p-4" name="image" type="file" accept="image/*"/>
                    <p class="text-sm text-on-surface-variant mt-3">Leave empty to keep the current image.</p>
                </section>
                ...
                <section class="bg-primary text-pure-white p-8 shadow-[0_32px_32px_rgba(32,42,54,0.04)] rounded-xl">
                    ...
                    <p class="text-sm text-surface-variant leading-6">This edit view mirrors the same product editor shell as the create screen, with the existing comic values bound for update.</p>
                </section>
            </div>
        </form>
    </main>
